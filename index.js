window.addEventListener("DOMContentLoaded", function () {
    ...
    const moreImage = document.querySelectorAll("#preview  .image ");
    const back = document.querySelectorAll(".fixed #back");
    const addTocart = document.querySelector("#preview #addtocart");
    ...
    signin.addEventListener("click", function () {
        if (document.querySelector("#profile #username").value != "" &&
    ...
    moreImage.forEach(function (img) {
        img.addEventListener("click", function () {
            const displayImage = document.querySelector(".displayImage");
            const image = img.querySelector("img");
            displayImage.setAttribute("src", image.getAttribute("src"));
            moreImage.forEach(function (img) {
    ...
    addTocart.addEventListener("click", function () {
        ...
        document.querySelector("#cart>.box>.two").innerHTML += `
        <div class="flex wrap inner">
            <div class="image one flex">
                <img src="${cartDetail.image}">
            </div>
            <div class="two flex col">
                <p class="brand-title side">${cartDetail.brandTitle}</p>
                <p class="price side">${"Price" + ": " + cartDetail.price}</p>
                <p class="quantity side">${"Quantity" + ": " + cartDetail.quantity}</p>
                <div class="flex inner wrap">
                    <button class="remove one">Remove</button>
                    <button class="view one">View</button>
                </div>
            </div>
        </div>`;

        const View = document.querySelectorAll("#cart .view");
        const Remove = document.querySelectorAll("#cart .remove");
        const Removeall = document.querySelector("#cart .removeall");
        const Buy = document.querySelector("#cart .buy");

        View.forEach(function (button) {
            button.addEventListener("click", function () {
                document.querySelector("#cart.fixed").style.display = "none";
                Viewbutton(Mobile);
                Viewbutton(Electronics);
                Viewbutton(Fashion);

                function Viewbutton(Type){
                    for (let i = 0; i < Type.length; i++) {
                        for (let j = 0; j < Type[i].length; j++) {
                            if (button.parentNode.parentNode.parentNode.querySelector("img").getAttribute("src") == Type[i][j].Image1) {
                                Preview(Type[i][j]);
                            };
                        };
                    };
        ...
        Remove.forEach(function (button) {
            button.addEventListener("click", function () {
                const Price = parseInt(this.parentNode.parentNode.querySelector(".price").innerHTML.replace("Price: ₹", ""));
                const Quantity = parseInt(this.parentNode.parentNode.querySelector(".quantity").innerHTML.replace("Quantity: ", ""));
        ...
        Removeall.addEventListener("click", function () {
            document.querySelector("#cart .two ").innerHTML = "";
            totalPrice = 0;
            updateTotal();
            Alert("Removed All Products");
        });

        Buy.addEventListener("click", function () {
            //function to update last order
            if (Issignin == true) {
                document.querySelector("#profile #totalprice .total").innerHTML = "₹" + " " + totalPrice;
                document.querySelector("#profile #totalprice .delivery").innerHTML = "₹" + " " + ((3 * (totalPrice / 100))).toFixed(2);
                document.querySelector("#profile #totalprice .totalpay").innerHTML = "₹" + " " + (totalPrice + (3 * (totalPrice / 100)));
                document.querySelector("#profile #totalprice .date-time").innerHTML = new Date().getDate() + "/" + (new Date().getMonth() + 1) + "/" + new Date().getFullYear() + " at " + new Date().getHours() + ":" + (new Date().getMinutes() > 10 ? new Date().getMinutes() : ("0" + new Date().getMinutes()));
            };
            setTimeout(function () {
                document.querySelector("#cart .two ").innerHTML = "";
                totalPrice = 0;
        ...
        (function () {
            var Productprice;
            var ProductQuantity;
            const productAdded = document.querySelectorAll("#cart .two ");
            productAdded.forEach(function (container) {
                Productprice = container.querySelector(".price").innerHTML.replace("Price: ₹", "");
                ProductQuantity = container.querySelector(".quantity").innerHTML.replace("Quantity: ", "");
            });
    ...
    changeBanner();
    product(document.querySelector("#mobile"), Mobile);
    product(document.querySelector("#fashion"), Fashion);
    product(document.querySelector("#electronics"),Electronics);

    // function to create product 
    function product(container, category) {
        for (let i = 0; i < category.length; i++) {
            container.innerHTML += `
            <div class="box grid-div-${[i + 1]}">
                <p class="side" id="${category[i][0].Heading.toLowerCase()}">${category[i][0].Heading.toUpperCase()}</p>
                <div class="product-container flex">
                </div>
            </div>`;

            const ProductContainer = container.querySelectorAll(".product-container");
            for (let j = 0; j < category[i].length; j++) {
                ...
                <div class="flex col">
                    ...
                    <div class="flex inner">
                        <div class="price one flex"></div>
                        <button class="preview one">View</button>
                    </div>
                </div>
            </div> `;};

            const Product = ProductContainer[i].querySelectorAll(".product");
            for (let k = 0; k < Product.length; k++) {
                Product[k].querySelector("img").setAttribute("src", category[i][k].Image1);
                Product[k].querySelector(".price").innerHTML = "₹ " + category[i][k].Price;
                Product[k].querySelector(".rating").innerHTML = stars.slice(5 - category[i][k].Rating, (5 - category[i][k].Rating) + 5);
            };
        };

        const Previewbtn = document.querySelectorAll(".preview");
        for (let i = 0; i < Previewbtn.length; i++) {
            Previewbtn[i].addEventListener("click", function () {
                for (let j = 0; j < category.length; j++) {
                    for (let k = 0; k < category[j].length; k++) {
                        if (Previewbtn[i].parentNode.parentNode.parentNode.querySelector("img").getAttribute("src") == category[j][k].Image1) {
                            Preview(category[j][k]);
                        };
                    };
                };
            });
    ...
    function Preview(Product) {
        ...
        container.querySelector(".displayImage").setAttribute("src", Product.Image1);
        container.querySelector(".rated").innerHTML = Product.Rating + "/5";
        container.querySelector(".stars").innerHTML = " " + stars.slice(5 - Product.Rating, (5 - Product.Rating) + 5);
        container.querySelector(".price").innerHTML = "₹ " + Product.Price;
        container.querySelector(".brand-title").innerHTML = Product.Brand + " | " + Product.Title;
        container.querySelector(".image1").setAttribute("src", Product.Image1);
        container.querySelector(".image2").setAttribute("src", Product.Image2);
        container.querySelector(".image3").setAttribute("src", Product.Image3);
        container.querySelector(".image4").setAttribute("src", Product.Image4);
        container.querySelector(".image5").setAttribute("src", Product.Image5);

        document.querySelector("#preview.fixed").style.display = ("flex");
    }
    ...
    function changeBanner() {
        ...
        const bannerImages = [
            "https://www.bycomercial.com/wp-content/uploads/2020/07/posts-Byc-115.jpg",
    ...
    //function for total, delivery, payable update
